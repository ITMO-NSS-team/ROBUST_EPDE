...
def bs_experiment(df, cfg, title):
    ...
    df_temp2 = (df_initial2[[col for col in df_initial2.columns if 'C' not in col]] != 0).copy()
    print(df_temp2.sum(axis=0).sort_values(ascending=False)[:len(cfg.params["fit"]["variable_names"])])
    init_nodes_list = []
    for i in range(len(cfg.params["fit"]["variable_names"])):
        init_nodes = df_temp2.sum(axis=0).idxmax()
        init_nodes_list.append(init_nodes)
        df_temp2 = df_temp2.drop(init_nodes, axis=1)
    print(init_nodes_list)

    for nodes in init_nodes_list:
        df.rename(columns={nodes: nodes[:-2] + '_r_' + nodes[-1]}, inplace=True)
